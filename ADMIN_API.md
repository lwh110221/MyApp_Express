...
| 参数名     | 类型   | 必填 | 说明     | 验证规则 |
|------------|--------|------|----------|----------|
| name       | string | 是   | 分类名称 | 1. 长度2-20个字符<br>2. 不能包含特殊字符 |
| code       | string | 是   | 分类代码 | 1. 长度2-20个字符<br>2. 只能包含小写字母、数字、下划线<br>3. 必须以字母开头 |
| sort_order | number | 否   | 排序     | 1. 最小值0<br>2. 值越小越靠前 |
...
#### 注意事项
1. 图片上传成功后会自动生成不同尺寸的缩略图
2. 图片URL格式：/uploads/news/YYYYMM/image-{random}.{ext}
3. 图片会自动进行压缩和优化处理
4. 不支持动图格式的缩略图生成
